...
def compare_execution_modes(df, ticker='AAPL'):
    ...
    for mode_name, data in results.items():
        logger.info(f"{mode_name:20s} | P&L: ${data['pnl']:>8.2f} | "
...
def demonstrate_almgren_chriss(df, ticker='AAPL'):
    ...
    for lambda_risk in [0.0, 0.01, 0.1]:
        ...
        logger.info(f"    First slice: {trajectory[0]:>6,} shares ({front_load_pct:>5.1f}%)")
        logger.info(f"    Schedule: {[int(x) for x in trajectory[:3]]}... "
                   f"(first 3 periods)")
...
def train_with_domain_randomization(df, ticker='AAPL'):
    ...
    logger.info(f"Realistic mode used: {realistic_count}/{n_episodes} episodes "
    ...
    logger.info("-" * 70)
    logger.info("✓ Agent learns strategies that work under BOTH perfect and imperfect execution")
