...
class SchwabTradingEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if signal > 0.33:  # BUY signal
            if prev_shares < 0:
                ...
                acct = self.client.get_account(self.account_hash).json()[
                    "securitiesAccount"
                ]
        ...
        if nav < 15000:  # Margin call threshold
            ...
            reward = -10.0
            logger.warning(
                f"Episode terminated: Account value below threshold (${nav:,.2f})"
            )

        # Log history
        self.history_log.append(
            {"nav": nav, "price": price, "step": self.client.current_step}
        )
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            logger.info(
                f"Step {self.client.current_step}: NAV = ${self.history_log[-1]['nav']:,.2f}"
            )
    ...
    def render_chart(self) -> None:
        ...
        try:
            ...
            ax1.plot(
                steps, prices, color="gray", alpha=0.6, linewidth=1.5, label="Price"
            )
            ...
            ax1.set_title(
                f"{self.ticker} Trading Performance", fontsize=14, fontweight="bold"
            )
            ax1.legend(loc="best")
